...
function createBinaryService(overrides: Record<string, unknown> = {}): {
...
function createSessionCwd(overrides: Record<string, unknown> = {}): any {
	const cwd = overrides.getCwd;
	return {
		getCwd: () => (typeof cwd === "function" ? cwd() : ((cwd as string | undefined) ?? "/tmp")),
...
suite("FooterManager", () => {
	...
	suite("sendFooterData", () => {
		...
		test("shortens cwd using home directory", () => {
			...
			assert.ok(
				notify.messages.some(
					(m: any) => m.type === "footer-data" && m.data.cwd === "~/projects/my-app",
				),
		...
		test("resolves to ~ when cwd is exactly home", () => {
			...
			const manager = new FooterManager(binaryService, notify);
			manager.sendFooterData(createSessionCwd({ getCwd: () => "/home/user" }));

			assert.ok(
				notify.messages.some((m: any) => m.data.cwd === "~"),
		...
		test("includes git branch", () => {
			...
			manager.sendFooterData(createSessionCwd({ getCwd: "/tmp" }));

			assert.ok(
				notify.messages.some((m: any) => m.data.gitBranch === "feature/add-api"),
		...
		test("includes session name", () => {
			const manager = new FooterManager(binaryService, notify);
			manager.sendFooterData(createSessionCwd({ getCwd: "/tmp", sessionName: "My Session" }));

			assert.ok(
				notify.messages.some((m: any) => m.data.sessionName === "My Session"),
		...
		test("notifies when values change", () => {
			const branchService = createBinaryService({
				resolveGitBranch: (cwd: string) => (cwd === "/tmp" ? "main" : "dev"),
			});
	...
	suite("start", () => {
		test("sends initial footer data", () => {
			...
			assert.ok(
				notify.messages.some((m: any) => m.type === "footer-data" && m.data.cwd === "/tmp"),
	...
	suite("stop", () => {
		test("clears interval", () => {
			...
			try {
				...
				manager.stop();
				assert.ok(intervalIds.length === 0, "expected interval to be cleared");
			} finally {
				(global as any).setInterval = originalSetInterval;
