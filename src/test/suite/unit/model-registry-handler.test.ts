...
function buildDeps(overrides: Partial<ModelRegistryHandlerDeps> = {}): ModelRegistryHandlerDeps {
...
let savedExecFileAsync: typeof shellModule.shellInternals.execFileAsync | undefined;
...
suite("ModelRegistryHandler", () => {
	...
	suite("getMergedModels", () => {
		test("returns empty array when registry unavailable", async () => {
			handler = new ModelRegistryHandler(buildDeps({ getModelRegistry: () => undefined }));
	...
	suite("buildModelList", () => {
		...
		test("falls back to id when name is missing", () => {
			handler = new ModelRegistryHandler(buildDeps());
			const result = handler.buildModelList([{ provider: "openai", id: "gpt-4" }]);
		...
		test("sorts by provider then name", () => {
			...
			assert.deepStrictEqual(
				result.map((m: any) => m.provider),
				["a", "a", "z"],
			);
			assert.deepStrictEqual(
				result.map((m: any) => m.name),
				["M1", "M2", "M3"],
			);
		});
	});
	...
	suite("refreshAvailableModels", () => {
		...
		test("updates list and sends models-updated", async () => {
			const registryModels = [{ provider: "openai", id: "gpt-4", name: "GPT-4" }];
			const registry: ModelRegistry = {
		...
		test("cache hit on second call after resolution", async () => {
			...
			try {
				handler = new ModelRegistryHandler(
					buildDeps({
						binaryService: {
							getBinaryPath: () => scriptPath,
						} as unknown as BinaryService,
					}),
				);
	...
	suite("invalidateCliModelIdsCache", () => {
		test("resets cache and promise", async () => {
			...
			try {
				...
				assert.ok(
					(handler as any).cliModelIdsCache,
					"cache should be set after resolution",
				);
	...
	suite("syncFavoritesToSettings", () => {
		test("no-op when no settingsManager", async () => {
			...
			handler = new ModelRegistryHandler(buildDeps({ getSettingsManager: () => undefined }));
	...
	suite("toggleFavorite", () => {
		...
		test("adds known model to favorites", async () => {
			...
			try {
				...
				assert.ok(
					written?.includes("openai/gpt-4"),
					"expected settingsManager to be updated",
				);
			} finally {
				await cleanupCliScript(scriptPath);
			}
	...
	suite("deriveAvailableThinkingLevels", () => {
		...
		test("returns only 'off' when reasoning is false", () => {
			assert.deepStrictEqual(deriveAvailableThinkingLevels({ reasoning: false }), ["off"]);
		});
