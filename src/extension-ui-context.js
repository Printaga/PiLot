...
exports.ExtensionUIContext = void 0;
class ExtensionUIContext {
    ...
    async bindExtensionUI() {
        ...
        try {
            ...
            session._extensionUIContext = uiContext;
            ...
            this.deps.logDebug(`[PI] Bound extension UI context for setStatus forwarding (${extensionCount} extensions loaded)`);
            // Re-apply bindings so the new UI context is used by the runner
            session._applyExtensionBindings?.(runner);
            ...
            const sessionStartEvent = session._sessionStartEvent ?? {
                type: "session_start",
                reason: "startup",
            ...
            session._sessionStartEvent = sessionStartEvent;
            this.deps.logDebug("[PiLot DIAGNOSTIC] Emitting session_start event:", sessionStartEvent);
            ...
            this.deps.logDebug(`[PiLot DIAGNOSTIC] Extension statuses after session_start: ${this.deps.extensionStatuses.size}`);
            if (this.deps.extensionStatuses.size > 0) {
                this.deps.logDebug("[PiLot DIAGNOSTIC] Statuses:", Object.fromEntries(this.deps.extensionStatuses));
            }
    ...
    forwardExtensionLoadingErrors() {
        ...
        try {
            ...
            if (!er.errors || er.errors.length === 0)
                return;
            for (const err of er.errors) {
                if (!err.path || !err.error)
                    continue;
                const errKey = `ext:error:${err.path}`;
                if (this.deps.extensionStatuses.has(errKey))
                    continue;
                ...
                const displayText = errorText.length > 200 ? errorText.slice(0, 200) + "…" : errorText;
    ...
    createUIContext() {
        return {
            ...
            custom: async (factory, options) => {
                ...
                this.deps.logDebug(`[PI] Extension custom UI (no TUI): factory=${factoryName}, caller=${callerLine}`);
                void options;
                ...
                return new Promise((resolve) => {
                    ...
                    try {
                        ...
                        if (component && typeof component?.then === "function") {
                            component.catch((e) => {
                                this.deps.logDebug(`[PI] Extension custom factory promise error: ${e}`);
                            });
                        }
                    }
                    catch (e) {
...
exports.ExtensionUIContext = ExtensionUIContext;
//# sourceMappingURL=extension-ui-context.js.map