<script lang="ts">
  ...
  let authCheckResults = $state<
    Record<string, { configured: boolean; credentialType: "oauth" | "api_key" | null }>
  >({});
  ...
  let loginResults = $state<Record<string, { success: boolean; message?: string; error?: string }>>(
    {},
  );
  ...
  onMount(() => {
    ...
    const onHostMessage = (event: MessageEvent) => {
      ...
      if (
        ...
        resetAddForm();
      } else if (msg.type === "error" && msg.data?.requestId === pendingRequestId) {
        addError =
          typeof msg.data?.message === "string" ? msg.data.message : "Failed to add provider";
        pendingAddId = null;
    ...
    const onCheckResult = (event: MessageEvent) => {
      ...
      if (typeof d.configured === "boolean") {
        ...
        if (d.configured) {
          const label = d.credentialType === "oauth" ? "OAuth active" : "API key valid";
          toast.showToast({
            type: "success",
            title: label,
    ...
    const onFetchModelsMessage = (event: MessageEvent) => {
      ...
      if (
        ...
        fetchError = null;
        pendingFetchId = null;
        const lines = (msg.data.models as Array<{ id: string; name?: string }>).map((m) =>
        ...
        pendingFetchId = null;
        fetchError =
          typeof msg.data?.message === "string" ? msg.data.message : "Failed to fetch models";
      }
    };
    ...
    const onLoginMessage = (event: MessageEvent) => {
      ...
      } else if (msg.type === "provider-login-result" && typeof d.provider === "string") {
  ...
  function parseModelLines(text: string): { id: string; name?: string }[] {
    ...
    // eslint-disable-next-line svelte/prefer-svelte-reactivity -- local dedupe set, intentionally non-reactive
  ...
  function statusLabel(status: string): string {
    switch (status) {
      ...
      case "models_json_key":
        return "Key from models.json";
      case "models_json_command":
        return "Command from models.json";
      case "configured":
        return "Configured";
      case "not_configured":
      default:
  ...
  <div class="header">
    <div class="header-row">
      ...
      <div class="header-actions">
        ...
        <button class="btn small" onclick={openAddForm} title="Add a custom provider">
          + Add provider
        </button>
        <button
      ...
      <div class="login-body">
        ...
        {#if login.userCode}
          <div class="login-device-code">
            ...
            <button class="btn small" onclick={() => copyText(login.userCode ?? "")}
              >Copy code</button
            >
          </div>
        {/if}
      ...
      <div class="login-body">
        <div class="login-title">
          <span class="auth-name"
            >{res.success ? "Logged in to" : "Login failed:"} {providerName}</span
          >
          <button class="icon-btn" onclick={() => dismissLoginResult(providerId)} title="Dismiss"
            >✕</button
          >
        </div>
        {#if res.error}
  ...
  {#if showAddForm}
    <div class="auth-card add-provider-card">
      <div class="auth-main">
        ...
        <div class="auth-info">
          <span class="auth-name"
            >{isEditingProvider ? "Edit custom provider" : "Add custom provider"}</span
          >
      ...
      <div class="add-provider-form">
        ...
        <label class="api-key-label">
          ...
          <div class="input-group">
            <input
              ...
              onkeydown={(e) => e.key === "Enter" && addProvider()}
            />
            <button class="vis-toggle" onclick={() => (showNewApiKey = !showNewApiKey)}>
              {#if showNewApiKey}
                <svg
                  width="14"
                  height="14"
        ...
        <label class="models-label">
          ...
          <textarea
            ...
            rows="3"
            placeholder="gpt-4o\nllama3.1:8b, Llama 3.1 8B"></textarea>
          <span class="field-hint"
            >Each line is a model ID, optionally "id, Display Name". Saved to models.json and shown
            in model selection.</span
          >
          <div class="fetch-models-row">
            ...
            {:else}
              <span class="field-hint"
                >Queries the OpenAI-compatible `{newProviderBaseUrl
                  .trim()
                  .replace(/\/+$/, "")}/models` endpoint. For authenticated providers, paste the API
                key above first — stored keys are never sent back to this form.</span
              >
            {/if}
          </div>
          {#if fetchError}
        ...
        {#if editingProvider === p.provider}
          <div class="auth-editor">
            <div class="input-group">
              ...
              />
              <button class="vis-toggle" onclick={() => (showApiKey = !showApiKey)}>
                {#if showApiKey}
                  <svg
                    width="14"
                    height="14"
        ...
        {:else}
          <div class="auth-actions">
            ...
            <button
              ...
              onclick={() => (p.custom ? startEditProvider(p) : startEditApiKey(p.provider))}
            >
              {p.configured ? "Update" : "Configure"}
            </button>
            {#if p.oauthLogin}
              {#if loginStates[p.provider]}
                <button class="btn small" onclick={() => cancelLogin(p.provider)}
                  >Cancel login</button
                >
              {:else}
                <button
