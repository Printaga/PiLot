...
exports.checkBetterSqlite3 = checkBetterSqlite3;
exports.rebuildBetterSqlite3 = rebuildBetterSqlite3;
exports.ensureBetterSqlite3Compatible = ensureBetterSqlite3Compatible;
exports.describeABIStatus = describeABIStatus;
/**
 ...
 */
function findBetterSqlite3Paths() {
    const candidates = [];
    ...
    const ciNested = path.join(piNpmDir, "@catdaemon", "pi-code-intelligence", "node_modules", "better-sqlite3");
 ...
 */
function checkBetterSqlite3(paths) {
 ...
 */
function tryPrebuildInstall(targetDir, electronVersion) {
    try {
        const result = (0, node_child_process_1.execSync)(`npx --yes prebuild-install --runtime electron --target ${electronVersion} --arch x64`, { cwd: targetDir, timeout: 30_000, maxBuffer: 256 * 1024 });
 ...
 */
function rebuildOnePath(targetDir) {
    const electronVersion = getElectronVersion();
    ...
    try {
        ...
        const result = (0, node_child_process_1.execSync)(`npx --yes node-gyp ${args.join(" ")}`, {
            cwd: targetDir,
    ...
    for (const copy of status.copies) {
        ...
        if (result.success) {
            ...
            if (verify === status.runtimeABI) {
                outputParts.push(`${path.basename(path.dirname(copy.dir))}/${path.basename(copy.dir)}: rebuilt OK`);
                continue;
            }
 ...
 */
function tryUpgradeToV12(brokenDir) {
    try {
        ...
        if (major >= 12)
            return false; // already v12+, not a v11 issue
 ...
 */
function ensureBetterSqlite3Compatible() {
    const status = checkBetterSqlite3();
    if (status.ok) {
        return { ok: true, rebuilt: false, output: "ABI compatible" };
    }
    const rebuildResult = rebuildBetterSqlite3();
    const anyRebuilt = rebuildResult.results.some((r) => r.success && !r.output.includes("already compatible"));
    return {
 ...
 */
function describeABIStatus(runtimeABI, moduleABI) {
